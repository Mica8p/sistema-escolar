...
export async function obtenerAdminesDisponibles() {
  try {
    ...
    return admins.map(admin => ({
      ...
      nombre: `${admin.persona.nombre} ${admin.persona.apellido}`,
      email: admin.persona.email,
      dni: admin.persona.dni,
      roles: admin.roles.map(r => r.rol.nombre),
      estado: admin.estado
    }));
  } catch (error) {
    console.error('[ERROR] obtenerAdminesDisponibles:', error);
    return [];
  }
}
...
export async function cederSuperAdminA(idUsuarioTarget: number) {
  const session = await auth();
  ...
  if (!esSuperAdminActual) {
    throw new Error('❌ Solo SUPER_ADMIN puede ceder el control');
  }
  ...
  try {
    ...
    if (!superAdminRol) {
      throw new Error('Rol SUPER_ADMIN no existe');
    }

    return await db.$transaction(async (tx) => {
      // 1. Asignar SUPER_ADMIN al usuario target
      ...
      console.log(
        `[AUDIT] ${session.user.email} cedió SUPER_ADMIN a ${usuarioTarget.persona.email}`
      );

      revalidatePath('/dashboard');
      return {
        success: true,
        message: `SUPER_ADMIN transferido a ${usuarioTarget.persona.nombre} ${usuarioTarget.persona.apellido}`,
        usuario: usuarioTarget
      };
    });
  } catch (error) {
    console.error('[ERROR] cederSuperAdminA:', error);
    throw error;
  }
}
