  ...
  try {
    return await db.$transaction(async (tx) => {
      ...
      if (asignacionesAnteriores.length === 0) {
        throw new Error("No se encontraron datos para copiar.");
      }
      ...
      for (const asig of asignacionesAnteriores) {
        const existe = await tx.asignacionAcademica.findFirst({
          where: {
        ...
        if (!existe) {
          ...
          if (asig.horarios.length > 0) {
            const nuevosHorarios = asig.horarios.map(h => ({
              idAsignacion: nuevaAsig.idAsignacion, // 🚩 El ID recién creado
      ...
      revalidatePath("/dashboard/horarios"); // 🚩 También refrescamos la vista de horarios

      return {
        success: true,
        count: asignacionesCreadas,
        horariosCount: horariosCreados
      };
    });
  } catch (error: any) {
    return { error: error.message || "Error en la clonación completa" };
  }
}