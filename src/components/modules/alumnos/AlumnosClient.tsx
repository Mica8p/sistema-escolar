...
const StatusBadge = ({ estado }: { estado: EstadoAcademico }) => {
  ...
  const statusLabels: Record<EstadoAcademico, string> = {
    Activo: "Activo",
  ...
  return (
    <span className={cn(baseClasses, statusStyles[estado])}>
      {statusLabels[estado]}
    </span>
  );
};
...
export function AlumnosClient({
  ...
  alumnos: AlumnoWithPersonaAndMatriculas[];
}) {
  const [search, setSearch] = useState("");
  const [selectedCurso, setSelectedCurso] = useState<number | null>(null);
  const [selectedStatus, setSelectedStatus] =
    useState<EstadoAcademico>("Activo");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5;

  const statusOptions = [
    { label: "Activos", value: "Activo" },
    { label: "Dado de Baja", value: "Retirado" },
  ...
  const filteredAlumnos = useMemo(() => {
    // Si no hay curso seleccionado, devolvemos un array vacío de inmediato
    if (selectedCurso === null) return [];

    return alumnos.filter((alumno) => {
      const searchLower = search.toLowerCase();
      const nombreCompleto =
        `${alumno.persona.nombre} ${alumno.persona.apellido}`.toLowerCase();
      ...
      // Filtrar por el curso seleccionado y el estado académico
      const matchesCursoYEstado = alumno.matriculas.some(
        (m) =>
          m.curso.idCurso === selectedCurso &&
          m.estadoAcademico === selectedStatus,
      );
    ...
    <>
      <div className="flex gap-4 mb-4 flex-col md:flex-row">
        ...
        <div className="flex-1">
          ...
          >
            {/* CAMBIO AQUÍ: Añadimos disabled y hidden */}
            ...
            <tbody className="divide-y divide-gray-100">
              {paginatedAlumnos.length > 0 ? (
                paginatedAlumnos.map((alumno) => {
                  // Buscamos la matrícula que corresponde específicamente al curso seleccionado
                  const matriculaActual = alumno.matriculas.find(
                    (m) => m.curso.idCurso === selectedCurso,
                  );
                    ...
                    >
                      ...
                      <td className="p-4">
                        {matriculaActual ? (
                          ...
                          >
                            {matriculaActual.curso.grado}° &quot;
                            {matriculaActual.curso.seccion}&quot; -{" "}
                            {matriculaActual.curso.turno}
                          </span>
                        ) : (
                          <span className="text-red-500 text-xs italic">
                            Sin matrícula en este curso
                          </span>
                        )}
                      </td>
                      <td className="p-4">
                        {matriculaActual && (
                          <StatusBadge
                            estado={matriculaActual.estadoAcademico}
                          />
                        )}
                      </td>
                ...
                <tr>
                  ...
                  >
                    {selectedCurso === null
                      ? "Por favor, seleccione un curso para visualizar el listado de alumnos"
                      : search.length > 0
                        ? "No se encontraron alumnos que coincidan con la búsqueda en este curso"
                        : "No hay alumnos inscriptos con el estado seleccionado en este curso"}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-slate-200 pt-4 mt-4 px-4 pb-4">
            <div className="text-sm text-slate-500">
              ...
              {Math.min(startIndex + itemsPerPage, filteredAlumnos.length)} de{" "}
              {filteredAlumnos.length} resultados
            </div>
